class App extends React.Component {
  ...
  componentDidMount() {
    setInterval(() => this.getGoogleMapData(), 800);
    setInterval(() => this.addListenerVignettes(), 800);
    setInterval(() => this.getCurrentDisplayedVignettes(), 500);
    setInterval(() => this.checkFilteredData(), 600);
  ...
  getGoogleMapData() {
    if (sessionStorage.getItem("searchdataLS") == null) {
      ...
      if (JSON.stringify(searchedData) != JSON.stringify(this.state.googleMapData) || this.state.googleMapData == "") {
        this.setState({googleMapData: searchedData, googleMapDataFiltered: searchedData});
        this.resetSelectedVignette();
      };
    };
  };
  ...
  resetSelectedVignette() {
    ...
    this.setState({savedSelectedstate: ObjectAllFalseSelectedVignettes});
  ...
  setPagesNumber() {
    ...
    if (nombrePages != this.state.nombrePages) {
      this.setState({nombrePages: nombrePages});
    };
    this.setCurrentPage();
  };
  ...
  saveCurrentState(nb, etat) {
    ...
    this.setState({savedSelectedstate: objectSelectedVignettes});
